...
void f_ls (struct file_directory *fd)
{
	...
	for(i=0; i<MAX_FILES; i++) {
		if (fd->file_table[i] != NULL) {
			...
			printf("%s :: %i %i/%i/%i %i:%i:%i :: %i %i/%i/%i %i:%i:%i\n", fd->file_table[i]->file_name, created->tm_wday, created->tm_mday, created->tm_mon, created->tm_year, created->tm_hour, created->tm_min, created->tm_sec,modified->tm_wday, modified->tm_mday, modified->tm_mon, modified->tm_year, modified->tm_hour, modified->tm_min, modified->tm_sec);
		}
	}
	return;
}
